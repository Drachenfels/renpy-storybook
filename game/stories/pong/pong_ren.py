...
class PongDisplayable(renpy.Displayable):
    def __init__(self):
        ...
        self.paddle = Solid(
            "#ffffff", xsize=self.PADDLE_WIDTH, ysize=self.PADDLE_HEIGHT
        )
    ...
    def render(self, width, height, st, at):
        ...
        if abs(self.by - self.computery) <= cspeed:
            ...
            self.computery += (
                cspeed * (self.by - self.computery) / abs(self.by - self.computery)
            )
        ...
        paddle(
            width - self.PADDLE_X - self.PADDLE_WIDTH,
            self.computery,
            width - self.PADDLE_X - self.PADDLE_WIDTH,
        )
        ...
        r.blit(
            ball,
            (int(self.bx - self.BALL_WIDTH / 2), int(self.by - self.BALL_HEIGHT / 2)),
        )

        # Check for a winner.
        if self.bx < -50:
            self.winner = "pong_player"
